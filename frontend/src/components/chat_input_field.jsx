...
export const ChatInputField = ({
  ...
  // makeResponse is a method to make response
  // disable is to disable the send button
  const [prompt, setPrompt] = useState("");

  return (
    <div className="shadow-sm w-full space-x-2 px-4">
      <div className="relative max-w-screen-lg mx-auto mb-2">
        ...
        <button
          ...
          className={`text-white absolute end-2.5 bottom-2.5 font-medium rounded-lg text-sm px-4 py-2 ${
            disable || String(prompt).trim().length == 0
              ? "bg-gray-300"
              : "bg-gray-700  hover:bg-gray-800"
          }`}
          onClick={
            !disable && String(prompt).trim().length != 0 && !loadingAnswer
              ? () => {
                  makeResponse(prompt);
                  setPrompt("");
                }
