...
async def create_new_chat(file: UploadFile , db: Session = Depends(database.get_db) , user: models.User = Depends(oauth2.get_current_user)):
    try:
        print("Check")
        ...
        while True:
            ...
            s3_key = f"{user.id}/{file_name}.{file.filename.split('.')[-1]}"
