...
@router.get("/login")
def get_login_user_data(user: models.User = Depends(oauth2.get_current_user) , db: Session = Depends(database.get_db)):
    return RedirectResponse(url=f"/users/{user.id}")
