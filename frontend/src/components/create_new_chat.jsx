import { FileInput } from "./file_input";
import { IoMdChatboxes } from "react-icons/io";
import { useState, useContext } from "react";
import { UserContext } from "../provider/user_provider";
import { useNavigate } from "react-router-dom";
import axios from "axios";
import { Toast } from "./toast";
import { ChatInputField } from "./chat_input_field";

export const CreateNewChat = () => {
  const [selectedFile, setSelecetedFile] = useState(null);
  const [toast, setToast] = useState({ visibility: false, message: "" });
  const [loading, setLoading] = useState(false);

  const { accessToken } = useContext(UserContext);

  const navigate = useNavigate();

  const selectFile = (file) => {
    setSelecetedFile(file);
    console.log(file);
  };

  const removeFile = () => {
    setSelecetedFile(null);
  };

  const createChat = async () => {
    if (!selectedFile) {
      alert("Please select a file from your device");
      return;
    }

    setLoading(true);

    const formData = new FormData();
    formData.append("file", selectedFile);

    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/chats`,
        formData,
        {
          headers: {
            Authorization: `bearer ${accessToken}`,
          },
        }
      );
      
      navigate(`/chat/${response.data.id}`);
    } catch (exception) {
      setToast({
        visibility: true,
        message: exception.response.data.detail,
      });

      console.log("Error Creating the chat");
    }

    setLoading(false);
  };

  const setToastVisiblity = (flag) => {
    setToast((prev) => ({
      ...prev,
      visibility: flag,
    }));
  };

  return (
    <>
      {toast.visibility && (
        <Toast
          setToastVisiblity={setToastVisiblity}
          message={toast.message}
          success={false}
        />
      )}

      <div className="flex flex-col w-full h-screen justify-between">
        {/* Chat Messages Will be displayed here */}
        <div className="h-full w-full overflow-y-scroll flex items-center justify-center">
          <div className="w-full max-w-screen-lg mx-auto h-full">
            <div className="m-4 h-full flex flex-col items-center justify-center space-y-2">
              <FileInput selectFile={selectFile} removeFile={removeFile} />
              <button
                className={`px-4 py-2 bg-gray-900 text-white rounded-md font-semibold flex items-center space-x-2 transition-all duration-300 ease-in ${
                  selectedFile == undefined || selectedFile == null
                    ? "bg-slate-300"
                    : "hover:scale-105 hover:shadow-xl"
                }`}
                onClick={selectedFile && !loading ? createChat : () => {}}
              >
                {loading ? (
                  <span>
                    <svg
                      aria-hidden="true"
                      role="status"
                      class="inline w-4 h-4 me-3 text-white animate-spin"
                      viewBox="0 0 100 101"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                        fill="#E5E7EB"
                      />
                      <path
                        d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                        fill="currentColor"
                      />
                    </svg>
                    Loading...
                  </span>
                ) : (
                  <>
                    <IoMdChatboxes className="h-6 w-6" />
                    <span>Create Chat</span>
                  </>
                )}
              </button>
            </div>
          </div>
        </div>

        {/* Input Field */}
        <ChatInputField disable={true} />
      </div>
    </>
  );
};
